      ...
      {rows.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <div className="text-[12px] text-muted-foreground mt-1">El calendario seed cubre 2026 — añade más eventos vía /api/promotions/webhook o edita src/lib/calendar/marketing-events.ts</div>
          </CardContent>
        </Card>
      ) : (
            ...
            return (
              <Card key={`${r.storeId}::${r.eventSlug}`} className="hover:border-[color:var(--accent)]/40 transition-colors">
                <CardContent className="p-3 flex items-center gap-3">
                  <div className="grid h-10 w-10 place-items-center rounded-md bg-secondary text-[12px] font-mono">
                    {r.sendDate.toISOString().slice(5, 10)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <div className="text-[13px] font-medium truncate">{r.eventName}</div>
                      <Badge variant="muted" className="text-[10px]">{r.pillar}</Badge>
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {r.draft && (
                      <Button variant="ghost" size="sm" asChild>
                        <Link href={`/campaigns/${r.draft.id}`} className="text-[11px]">Ver →</Link>
                      </Button>
                    )}
                  </div>
                </CardContent>
