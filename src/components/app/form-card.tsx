        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <Link href={`/forms/${form.slug}`} className="text-[13px] font-medium hover:underline truncate block">{form.name}</Link>
              <div className="flex items-center gap-1.5 mt-0.5">
                <Badge variant="muted" className="text-[10px]">{form.kind}</Badge>
                {form.store && <Badge variant="muted" className="text-[10px]">{form.store.name}</Badge>}
                <Badge variant={form.status === "PUBLISHED" ? "positive" : "muted"} className="text-[10px]">
